...
**Branch**: ${branchName}<br />**Implementation Timespan**: ${this.formatDate(startDate)} - ${this.formatDate(endDate)}<br />**Breaking Changes**: ${versions.major.length}<br />**Features**: ${versions.minor.length}<br />**Fixes**: ${versions.patch.length}<br />**Total Commits**: ${(versions.major.length+versions.minor.length+versions.patch.length+versions.invalid.length)}
...
${majorChanges}${minorChanges}${patchChanges}${invalidChanges}
        ...
        getLatestSemverFromTags(tagList) {
            if (tagList && tagList.length) {
                const highest = tagList[0].name;
                let index = 1;

                for (let i = 1, l = tagList.length; i < l; i++) {
                    if (semver.valid(tagList[i].name) && semver.gt(tagList[i].name, highest)) {
                        highest = tagList[i].name;
                    }
                }

                if (semver.valid(highest.name)) return highest;
            }
